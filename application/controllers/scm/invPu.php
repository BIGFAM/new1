...
class InvPu extends CI_Controller {
	...
	public function add(){
	    ...
	    $currData = (array)json_decode($data, true);//当前的单据编号
	    $currBn = strval($currData['billNo']);
		if (strlen($data)>0) {
			...
			$info = elements(array(
				...
				'accId',
				'modifyTime'
				),$data);//elements返回的是一个数组
			$finalBn = strval($info['billNo']);//最终的单据编号
			$conn = @mysql_connect("localhost","root");
			mysql_select_db("invoicing", $conn);
			$sql = "UPDATE ci_contract_img SET invId = \"".$finalBn."\" WHERE invId = \"".$currBn."\" ";
			mysql_query($sql,$conn);//更新单据编号
			$this->db->trans_begin();
