...
class Inventory extends CI_Controller {
	...
	//上传合同图片信息
	public function uploadImages88() {
	    require_once './application/libraries/UploadHandler.php';
		$config = array(
			'script_url' => base_url().'inventory/uploadimages88',
		...
		$uploadHandler = new UploadHandler($config);
		$list  = (array)json_decode(json_encode($uploadHandler->response['files'][0]), true); 
		$newid = $this->mysql_model->insert(contract_IMG,$list);
		$files[0]['pid']          = intval($newid);
		$files[0]['status']       = 1;
		$files[0]['size']         = (float)$list['size'];
		$files[0]['name']         = $list['name'];
		$files[0]['url']          = site_url().'/basedata/inventory/getImage88?action=getImage88&pid='.$newid;
		$files[0]['thumbnailUrl'] = site_url().'/basedata/inventory/getImage88?action=getImage88&pid='.$newid;
		$files[0]['deleteUrl']    = '';
		$files[0]['deleteType']   = '';
		$data['status'] = 200;
		$data['msg']    = 'success';
	...
	//保存上传合同图片信息
	public function addImagesToInv88() {
	    $data = $this->input->post('postData');
		if (strlen($data)>0) {
		    $v = $s = array();
		    $data = (array)json_decode($data, true); 
			...
			$this->mysql_model->update(contract_IMG,array_values($v),'id');
			$this->mysql_model->update(contract_IMG,array_values($s),'id');
			str_alert(200,'success'); 
	    }
		str_alert(-1,'保存失败'); 
	}
	...
	public function getImage88() {
	    $id = intval($this->input->get_post('pid',TRUE));
	    $data = $this->mysql_model->get_row(contract_IMG,'(id='.$id.')');
		if (count($data)>0) {
