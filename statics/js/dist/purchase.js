	...
	THISPAGE = {
		...
		loadGrid: function(a) {
			...
			this.calAmount = "amount", taxRequiredCheck && (v.pop(), v.push({
				...
				align:"center",
				formatter:function(){
					var zz = '<div class="operating" data-id="3"><span class="ui-icon ui-icon-pic" title="合同图片"></span></div>';
					return zz;
				}
			},
			// { 参考用代码
		...
		addEvent: function() {
			...
			this.$_date.bind("keydown", function(a) {
				...
				this.select()
			}),$("#grid").on("click", ".operating .ui-icon-pic", function(a) {//点击上传图按钮修改！！！
			a.preventDefault();
			var b = $(this).parent().data("id"),
				bb = $("#number").html(),
				c = "商品图片";
			$.dialog({
				content: "url:../settings/fileUpload88",
				data: {
					title: c,
					id: bb,
					callback: function() {}
			...
			}), $(".wrapper").on("click", "#savaAndAdd", function(b) {//保存并新增修改
				b.preventDefault();
				var c = $(this),
					d = THISPAGE.getPostData();
